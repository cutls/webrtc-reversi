...
import { type IBoard, type IConfig, type IMode, fullZeroBoard, getZeroBoard, initBoard, initailConfig } from '@/type'
import { isWinner, passCheck, reversi } from '@/utils/game'
import { TimerContext } from '@/utils/timer'
import { create } from '@/utils/webrtc'
import { Alert, AlertIcon, Box, Button, Container, Flex, Input, Progress, Text } from '@chakra-ui/react'
	...
	return (
		<>
			...
			<Container p={2}>
				...
				{config.haveTime > 0 && state === 'playing' && (
					<Flex mt={5}>
						<Text>
							あと
							<Text as="span" style={{ fontVariantNumeric: 'tabular-nums' }}>
								{timer?.timer && timer?.timer > 0 ? Math.floor(timer?.timer) : '-'}
							</Text>
							秒
						</Text>
						<Box w={200} ml={2} pt={1}>
