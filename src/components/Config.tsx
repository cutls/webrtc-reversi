import type { IConfig, IState } from '@/type'
...
export default function Footer({ config, setConfig, start }: IProps) {
	return (
		<>
			...
			<Flex align="center">
				<Button isDisabled={config.rows <= 4} onClick={() => setConfig({ ...config, rows: config.rows - 2 })}>
					-
				</Button>
			...
			<Flex align="center">
				<Button isDisabled={config.haveTime <= 5} onClick={() => setConfig({ ...config, haveTime: Math.ceil(config.haveTime / 5) * 5 - 5 })}>
					-5秒
				</Button>
				<Text w={12} textAlign="center">
					{config.haveTime < 0 ? 'なし' : `${config.haveTime}秒`}
				</Text>
				<Button onClick={() => setConfig({ ...config, haveTime: Math.ceil(config.haveTime / 5) * 5 + 5 })}>+5秒</Button>
				<Button isDisabled={config.haveTime === -1} onClick={() => setConfig({ ...config, haveTime: -1 })} colorScheme="teal" ml={2}>
					なし(無制限)
				</Button>
			</Flex>
			<Button size="lg" colorScheme="blue" onClick={() => start()} mt={10}>
				開始
			</Button>
		</>
	)
