import type { IBoard, IConfig } from '@/type'
...
export default function Footer({ config, board, placeable, turn, place, iAmBlack }: IProps) {
		...
		Array.from({ length: rows }).map((_, j) =>
			Array.from({ length: rows }).map((_, i) => {
				...
				const elm = (
					...
					<Flex key={`${i}-${j}`} h={`${aWidth}px`} w={`${aWidth}px`} bg="#36754a" justifyContent="center" alignItems="center">
						{current > 0 && <Box h={`${aWidth * 0.8}px`} w={`${aWidth * 0.8}px`} bg={current === 1 ? '#000' : '#fff'} borderRadius={`${aWidth * 0.4}px`} />}
						{able && (
							<Box
								onTouchStart={() => place(i, j)}
								onClick={() => place(i, j)}
								h={`${aWidth * 0.8}px`}
								w={`${aWidth * 0.8}px`}
								bg={turn === 1 ? '#000' : '#fff'}
								opacity={showPlaceable ? 0.3 : 0}
								borderRadius={`${aWidth * 0.4}px`}
							/>
						)}
					</Flex>
				)
	...
	return (
		<>
			<Text>
				{turn === 1 ? '黒' : '白'}のターン(黒: {board.flat().filter((v) => v === 1).length} / 白: {board.flat().filter((v) => v === 2).length})
			</Text>
			...
			<Text>
				{isMyTurn ? 'あなたのターン' : '相手のターン'}(あなたは{iAmBlack ? '黒' : '白'}です)
			</Text>
		</>
	)
}
