import type { IBoard } from '@/type'

export const reversi = (rows: number, x: number, y: number, dx: number, dy: number, target: 1 | 2, board: IBoard, putMode?: boolean) => {
	// target: myColor
...
export const passCheck = (rows: number, board: IBoard, placeableOld: IBoard, target: 1 | 2) => {
	...
	const a = [-1, 0, 1]
	for (let i = 0; i < rows; i++) for (let j = 0; j < rows; j++) for (const x of a) for (const y of a) if (!board[i][j] && reversi(rows, i, j, x, y, target, board)) placeable[i][j] = 1
	return placeable
}
